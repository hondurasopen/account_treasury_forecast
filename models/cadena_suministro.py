...
class DeliveFacturas(models.Model):
    _name = 'account.delivery.forecast.product'
    ...
    treasury_id = fields.Many2one("account.delivery.forecast", "Forecast")
    porcentaje_sale = fields.Float("Ventas %")
    porcentaje_unidades = fields.Float("Producto %")


class DeliveFacturas(models.Model):
    _name = 'account.delivery.forecast.invoice'
...
class AccountTreasuryForecast(models.Model):
    _name = 'account.delivery.forecast'
    ...
    out_invoice_ids = fields.One2many("account.delivery.forecast.invoice", "treasury_id", "Facturas de Clientes")

    product_forecast_ids = fields.One2many("account.delivery.forecast.product", "treasury_id", "Venta de Productos")
    ...
    def get_product(self):
        ...
        obj_forecast_product = self.env["account.delivery.forecast.product"]
        treasury_invoice_obj = self.env['account.delivery.forecast.invoice'].search([('treasury_id', '=', self.id)])
    ...
    def calculate_invoices(self):
        ...
        treasury_invoice_obj = self.env['account.delivery.forecast.invoice']
        state = []
